from typing import List, Optional


# Definition for singly-linked list.
class ListNode:
    def __init__(self, val=0, next=None):
        self.val = val
        self.next = next


class Solution:
    def mergeKLists(self, lists: List[Optional[ListNode]]) -> Optional[ListNode]:
        # Base case: if the input is empty or null, return None
        if not lists:
            return None

        # Keep merging lists until only one remains
        while len(lists) > 1:
            merged_lists = []
            # Merge lists pairwise
            for i in range(0, len(lists), 2):
                l1 = lists[i]
                l2 = lists[i + 1] if (i + 1) < len(lists) else None
                merged_lists.append(self.mergeTwoLists(l1, l2))
            lists = merged_lists

        # Return the final merged list
        return lists[0]

    def mergeTwoLists(self, l1: Optional[ListNode], l2: Optional[ListNode]) -> Optional[ListNode]:
        # Create a dummy node to start the merged list
        dummy = ListNode()
        current = dummy

        # Merge the two lists
        while l1 and l2:
            if l1.val < l2.val:
                current.next = l1
                l1 = l1.next
            else:
                current.next = l2
                l2 = l2.next
            current = current.next

        # Append any remaining nodes
        if l1:
            current.next = l1
        if l2:
            current.next = l2

        # Return the merged list (excluding the dummy node)
        return dummy.next

def list_to_linked_list(arr):
    dummy = ListNode(0)
    current = dummy
    for val in arr:
        current.next = ListNode(val)
        current = current.next
    return dummy.next

def linked_list_to_list(head):
    result = []
    current = head
    while current:
        result.append(current.val)
        current = current.next
    return result

def test_merge_k_lists():
    solution = Solution()

    # Test case 1: Normal case
    lists1 = [list_to_linked_list([1,4,5]), list_to_linked_list([1,3,4]), list_to_linked_list([2,6])]
    print("Test 1: [[1,4,5],[1,3,4],[2,6]]")
    result1 = solution.mergeKLists(lists1)
    print(f"Result: {linked_list_to_list(result1)}")
    print(f"Expected: [1,1,2,3,4,4,5,6]\n")

    # Test case 2: Empty list
    lists2 = []
    print("Test 2: []")
    result2 = solution.mergeKLists(lists2)
    print(f"Result: {linked_list_to_list(result2)}")
    print(f"Expected: []\n")

    # Test case 3: Single empty list
    lists3 = [list_to_linked_list([])]
    print("Test 3: [[]]")
    result3 = solution.mergeKLists(lists3)
    print(f"Result: {linked_list_to_list(result3)}")
    print(f"Expected: []\n")

    # Test case 4: Lists of different lengths
    lists4 = [list_to_linked_list([1]), list_to_linked_list([0,2]), list_to_linked_list([-1,3,4,5,6])]
    print("Test 4: [[1],[0,2],[-1,3,4,5,6]]")
    result4 = solution.mergeKLists(lists4)
    print(f"Result: {linked_list_to_list(result4)}")
    print(f"Expected: [-1,0,1,2,3,4,5,6]\n")

if __name__ == '__main__':
    test_merge_k_lists()